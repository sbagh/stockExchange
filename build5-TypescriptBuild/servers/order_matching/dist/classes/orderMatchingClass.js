...
exports.orderMatchingClass = void 0;
class orderMatchingClass {
    ...
    removeOrder(orderID, orderType) {
        if (orderType === "buy") {
            const index = this.buyOrders.findIndex((order) => order.orderID === orderID);
            if (index !== -1) {
            ...
            const index = this.sellOrders.findIndex((order) => order.orderID === orderID);
            if (index !== -1) {
    ...
    matchOrders() {
        ...
        while (i < this.buyOrders.length && j < this.sellOrders.length) {
            if (this.buyOrders[i].ticker === this.sellOrders[j].ticker &&
                this.buyOrders[i].price >= this.sellOrders[j].price) {
                let trade_quantity = Math.min(this.buyOrders[i].quantity, this.sellOrders[j].quantity);
...
exports.orderMatchingClass = orderMatchingClass;
//# sourceMappingURL=orderMatchingClass.js.map